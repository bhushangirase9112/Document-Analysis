...
# llm = LLM(model="groq/llama-3.3-70b-versatile", response_format=Dog)
...
# Extract structured results for job_store

# Updated to handle a list of dicts: [summary_dict, entities_dict, sentiment_dict]
def extract_structured_results(result_list):
    summary = None
    entities = None
    sentiment = None
    if isinstance(result_list, list):
        # Find dict with 'summary' key
        for item in result_list:
            if isinstance(item, dict) and 'summary' in item:
                summary = item['summary']
            elif isinstance(item, dict) and all(k in item for k in ['people', 'organizations', 'dates', 'locations']):
                entities = item
            elif isinstance(item, dict) and all(k in item for k in ['tone', 'confidence']):
                sentiment = item
    # Fallbacks if not found
    summary = summary if summary is not None else "Summary unavailable"
    entities = entities if entities is not None else {"people": [], "organizations": [], "dates": [], "locations": []}
    sentiment = sentiment if sentiment is not None else {"tone": "neutral", "confidence": 0.0}
